...
class TApplication(tk.Frame):
    def __init__(self, args, logger, master):
        ...
        self.basic_forms = read_folder(TCollection.basic)
        self.vehicles_forms = read_folder(TCollection.vehicles)
        self.geometric_forms = [TSpriteForm(TCollection.geometric, form=f, color=c) for c in ["blue", "red", "yellow", "black"] \
                                    for f in ["rectangle", "triangle", "circle", "pentagram"] ]
        self.russian_abc = [TSpriteForm(TCollection.russian_abc, char=a) for a in "АЕИЯЮЭЁАЕИЯЮЭЁАБВГДЕЁЖЗИКЛМНОПРСТУФХЦЧШЩЭЮЯ"]
                                                        # three times vowels
        self.bird = read_folder(TCollection.birds)
        ...
        self.master.bind('<Control-N>', self.init_new_game)
        self.master.bind("<Button-1>", self.init_new_game)
        ...
        self.canvas.update()
        start_x = (self.canvas.winfo_width() - TSpriteForm.sprite_width) / 2
        start_y = self.canvas.winfo_height() - TSpriteForm.sprite_height - 2
        self.start_point = TPoint(start_x, start_y)
    ...
    def check_end_of_game(self):
        for g in self.goals:
            ...
            if m is not None:
                ...
                self.move_mouse_to_start_position()
                if m:
    ...
    def move_mouse_to_start_position(self):
        #sys.stderr.write("move_mouse_to_start_position\n".format())
        self.move_mouse(self.start_point.x, self.start_point.y)
    ...
    def init_new_game(self, event=None):
        ...
        self.sprite.point = TPoint(self.start_point.x, self.start_point.y)
